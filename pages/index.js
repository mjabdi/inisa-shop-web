...
import { FormatColorText, FormatUnderlined, Rotate90DegreesCcw } from '@material-ui/icons'
...
const Home = ({ t }) => {
  ...
  const [hpcAnimationClasses, setHpcAnimationClasses] = React.useState(clx(styles.hpc_animate))
  const [hpcAnimationClasses2, setHpcAnimationClasses2] = React.useState(clx(styles.hpc_animate))
  const [hpcAnimationClasses3, setHpcAnimationClasses3] = React.useState(clx(styles.hpc_animate))
  ...
  React.useEffect(() => {
    ...
    const sloganTimer = setInterval(() => {
      setSloganIndex(prev => {
        ...
        {
          setSloganArrayIndex(prev => prev < sloganArray.length - 1 ? prev + 1 : 0)
          return 0
        }
    ...
    setHpcAnimationClasses(clx(styles.hpc_animate_from_left,styles.hpc_animate_delay_1 ))
    setHpcAnimationClasses2(clx(styles.hpc_animate_from_left,styles.hpc_animate_delay_2 ))
    setHpcAnimationClasses3(clx(styles.hpc_animate_from_left,styles.hpc_animate_delay_3 ))
  ...
  const handleScroll = (event) =>
  {
    ...
    setHpc_piscs_x(scrollTop * 8)
    setHpc_piscs_y(-scrollTop * 5)
      ...
      >
        <Grid item xs={12} id="1" >
          <div className={styles.grid1}>
            ...
            >
              <Grid item id="sec11" xs={12} md={6}>
                <div className={styles.sec11}>
                  <h1 className={hpcAnimationClasses}>
                    <div>همین حالا آغاز کنید</div>
                    <div>
                   
                        <span className={clx(styles.hpc_underlined,getCurrentSlogan().bg)}>{getSloganPart()}</span>
                        <span className={clx(styles.hpc_caret,styles.hpc_caret_show,styles.hpc_caret_blink)}></span>
                
                    </div>
                  </h1>
                  <p className={hpcAnimationClasses2}>
                    با ما فروش آنلاین را تجربه کنید. با اینیسا، فروشگاه رایگان خود را برای همیشه داشته باشید. یکبار ثبت نام کنید و همیشه استفاده کنید.
                  </p>
                  <div className={hpcAnimationClasses3}>
                    <Button
                        style={{ backgroundColor: "#111111", color: "#fff", width: "200px", padding: "10px", marginRight:"250px" }}
                        type="button"
                        variant="contained"
                      >
                        رایگان فروشگاه بساز
              ...
              <Grid item id="sec12" xs={12} md={6}>
                ...
                <div 
                  ...
                  style={{transform:`rotate(${hpc_phone_degree}deg) translateX(${hpc_phone_x}px) translateY(${hpc_phone_y}px)`}}> 
                   <Image className={clx(styles.hpc_phone__slide_current)}  src="/images/phone-slide1_.jpg" width="250px" height="450px" alt="phone-pic"/>
                </div>
                ...
                <div className={clx(styles.hpc_pics_container)} style={{transform:`rotate(${hpc_pics_degree}deg) translateX(${hpc_pics_x}px) translateY(${hpc_pics_y}px)`}}>
                   <div className={clx(styles.hpc_pics,hpc_pics_bg[getCurrentSloganIndex()])}>   </div>
                </div>
